...
if [ "\$ARCH" = "arm" ]  || [ "\$ARCH" = "arm64" ]; then
  ...
  if [ "\$IS64BIT" = true ]; then
    ...
    mv "\$TMP_MODULE_DIR/libgadget-arm64.so.xz" "\$TMP_MODULE_DIR/${gadgetLibraryName}.xz"
  else
    extract "\$ZIPFILE" "gadget/libgadget-arm.so.xz" "\$TMP_MODULE_DIR"
    mv "\$TMP_MODULE_DIR/libgadget-arm.so.xz" "\$TMP_MODULE_DIR/${gadgetLibraryName}.xz"
  fi
fi
...
if [ "\$ARCH" = "x86" ] || [ "\$ARCH" = "x64" ]; then
  ...
  if [ "\$IS64BIT" = true ]; then
    ...
    mv "\$TMP_MODULE_DIR/libgadget-x64.so.xz" "\$TMP_MODULE_DIR/${gadgetLibraryName}.xz"
  else
    extract "\$ZIPFILE" "gadget/libgadget-x86.so.xz" "\$TMP_MODULE_DIR"
    mv "\$TMP_MODULE_DIR/libgadget-x86.so.xz" "\$TMP_MODULE_DIR/${gadgetLibraryName}.xz"
  fi
fi
